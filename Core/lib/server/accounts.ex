defmodule Server.Accounts do
  ...
  # Need to refactor authorize to be check_magic_link
  def authorize(magic_link) when is_bitstring(magic_link) do
    query =
      from a in AuthToken,
        # Logic for Magic Link duration. SHOULD be a macro or constant. Not burried in accounts.ex
        where: a.inserted_at > from_now(-5, "minute") and a.value == ^magic_link
